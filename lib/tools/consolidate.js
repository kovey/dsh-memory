...
export function consolidateTool(deps) {
    return defineTool({
        name: 'memory_consolidate',
        description: 'Run a memory-quality pass over one scope: expire and archive stale lessons, halve confidence of unreinforced records, detect contradicting lessons, and propose skills for repeatedly-verified lessons. Dry-run by default — pass dryRun=false to apply. Promotion proposals are always reported, never executed.',
        parameters: {
            scope: {
                type: 'string',
        ...
        async execute(args, exec) {
            ...
            const applies = args.dryRun === false || args.acceptProposal !== undefined || args.rejectProposal !== undefined;
            if (applies) {
            ...
            if (args.acceptProposal !== undefined || args.rejectProposal !== undefined) {
                ...
                for (const scope of targets) {
                    ...
                    if (changed > 0)
                        lines.push(`${status} proposal for ${target} (${scope.kind}) — now write the skill yourself, then it is done`);
                }
...
export function forgetTool(deps) {
    return defineTool({
        name: 'memory_forget',
        description: 'Retire one memory record (archive it, keeping the file under archive/lessons for review). Use when a lesson turned out to be wrong or obsolete, or when the user asks to forget something.',
        parameters: {
...
function resolveTargets(deps, agent, scope) {
    ...
    const targets = scope === 'global' ? [] : [deps.resolver.resolve({ agent })];
    if (scope === 'global' || scope === 'all') {
        const global = deps.resolver.globalScope();
        if (!targets.some((candidate) => candidate.root === global.root))
            targets.push(global);
    }
    return targets;
}
/** Used by tests: accept a proposal without going through the tool surface. */
export function acceptProposal(deps, scope, recordId) {
    const store = deps.registry.open(scope);
    if (store === undefined)
        return 0;
