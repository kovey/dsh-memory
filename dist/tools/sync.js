...
export function syncTool(deps) {
    return defineTool({
        name: 'memory_sync',
        description: 'Synchronize a memory root with its git remote: commit local changes, fetch, rebase, resolve text-view conflicts by rule, and rebuild the database from the text view. Pushing only happens when push=true is passed explicitly.',
        parameters: {
...
function syncOne(deps, scope, options) {
    ...
    try {
        ...
        if (!hasRemote(scope.root)) {
            ...
            if (options.rebuild)
                lines.push(...rebuildLine(deps, scope));
            return lines;
        }
        ...
        if (outcome.action === 'conflict') {
            ...
            const resolutions = outcome.conflicts.map((file) => {
            ...
            for (const item of resolutions) {
                if (item.strategy === 'manual' || item.content === undefined)
                    continue;
...
function rebuildLine(deps, scope) {
    const store = deps.registry.open(scope);
    if (store === undefined)
        return ['  rebuild: skipped (store unavailable)'];
    try {
...
function resolveTargets(deps, agent, scope) {
    ...
    const targets = globalOnly ? [] : [deps.resolver.resolve({ agent })];
