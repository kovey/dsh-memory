...
export function sessionsDir(scope: MemoryScope): string {
    return path.join(scope.root, 'sessions')
}

/** Episode file name: date-stamped and session-scoped. */
export function sessionFilePath(scope: MemoryScope, sessionId: string, now = new Date()): string {
...
export function recordEpisode(db: DatabaseSync, scope: MemoryScope, input: EpisodeInput, now = new Date()): number {
...
export function episodeDigest(db: DatabaseSync, sinceDays = 90): EpisodeDigest {
