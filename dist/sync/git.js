...
function run(cwd, args, timeoutMs = 15_000) {
    try {
...
function realPathOf(target, cache = new Map()) {
...
export function ensureRepo(root) {
...
export function isBusy(root) {
    ...
    return ['rebase-merge', 'rebase-apply', 'MERGE_HEAD', 'CHERRY_PICK_HEAD'].some((marker) => fs.existsSync(path.join(state, marker)));
}
/**
...
export function commitMemory(root, options) {
    const target = relativePaths(root);
    if (target === undefined)
        return { ok: false, stdout: '', stderr: 'not a git repository', code: 1, committed: false, files: 0 };
    if (isBusy(root)) {
    ...
    if (!add.ok)
        return { ...add, committed: false, files: 0 };
    const status = run(target.repo, ['diff', '--cached', '--name-only', '--', ...paths]);
    const files = status.stdout.split('\n').filter((line) => line.trim() !== '');
    if (files.length === 0)
        return { ok: true, stdout: '', stderr: '', code: 0, committed: false, files: 0 };
...
export function sync(root, options = {}) {
    ...
    if (!fetch.ok)
        return { ok: false, action: 'error', detail: `fetch failed: ${fetch.stderr.trim()}`, conflicts: [], rebased: false };
    ...
    if (options.push === true) {
        ...
        if (!push.ok)
            return { ok: false, action: 'error', detail: `push failed: ${push.stderr.trim()}`, conflicts: [], rebased: true };
...
export function continueRebase(root, resolvedPaths) {
    const target = relativePaths(root);
    if (target === undefined)
        return { ok: false, stdout: '', stderr: 'not a git repository', code: 1 };
