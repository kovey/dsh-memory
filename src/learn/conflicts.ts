 ...
 */
export function directivePolarity(body: string): -1 | 0 | 1 {
    ...
    let positive = 0
    for (const marker of POSITIVE) if (stripped.includes(marker)) positive += 1
...
export function sharedObjects(a: string, b: string): string[] {
    const left = new Set(extractTerms(a, 40))
    const right = new Set(extractTerms(b, 40))
    const shared: string[] = []
    for (const token of left) if (right.has(token) && token.length >= 3) shared.push(token)
    return shared
}

/** Detect contradictions among active records. Pure read. */
export function detectConflicts(records: readonly MemoryRecord[]): ConflictCandidate[] {
    ...
    for (let i = 0; i < records.length; i += 1) {
        for (let j = i + 1; j < records.length; j += 1) {
            ...
            if (topic < TOPIC_THRESHOLD) continue
 ...
 */
export function pickWinner(a: MemoryRecord, b: MemoryRecord): MemoryRecord {
    const evidenceDiff = a.evidence.length - b.evidence.length
    if (Math.abs(evidenceDiff) >= 2) return evidenceDiff > 0 ? a : b
    const confDiff = a.confidence - b.confidence
    if (Math.abs(confDiff) > 0.1) return confDiff > 0 ? a : b
...
export function applyConflicts(
    ...
    options: { resolve?: boolean; now?: Date } = {},
): ConflictRecord[] {
    ...
    for (const candidate of candidates) {
        ...
        if (!already) insert.run(candidate.winner.id, candidate.loser.id, candidate.reason, now.toISOString())
        let resolved = false
...
export function conflictCount(db: DatabaseSync): number {
...
export function activeRecords(db: DatabaseSync, limit = 500): MemoryRecord[] {
