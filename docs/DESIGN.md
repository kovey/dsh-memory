...
| # | 要求 | 本设计的回答 |
|---|---|---|
| 1 | TypeScript 实现 | 纯 TS + ESM，`tsc` 构建到 `dist/`，零运行期第三方依赖 |
...
### 14.12 增长边界与保留策略（审查后补齐）

用户问题：「记忆会不断膨胀导致上下文爆炸吗？」答案分两侧，均已落成机制与文档：

- **上下文侧 O(1)**：常驻段 ≤590 tok（协议 240 + 索引 150 + L5 200）、自动召回 ≤600 tok/轮且同会话幂等、
  工具单次 ≤4000 tok 且**新增会话累计预算**（`recall.sessionToolBudgetTokens`，默认 20k，防循环调用）、
  L1 永不自动注入。实测 55 次注入：中位 234 / 最大 592 tok。
