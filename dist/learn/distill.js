 ...
 */
export function resolveDistillRoute(config, agent) {
...
export function dailyDistillTokens(db, now = new Date()) {
...
export function distillAllowed(deps, request) {
    const learn = deps.config.learn;
    if (!learn.autoDistill)
        return { allowed: false, reason: 'autoDistill disabled' };
    if (request.signals.length < Math.max(1, learn.minSignals))
        return { allowed: false, reason: 'no pain signals' };
    if (!deps.resolver.mayWrite(request.agent))
        return { allowed: false, reason: 'subagent sessions do not author memory' };
    const budget = deps.state.distillBudget(request.sessionId);
    if (budget.runs >= learn.maxDistillPerSession)
        return { allowed: false, reason: 'session distillation budget exhausted' };
    const store = deps.registry.open(deps.resolver.resolve({ agent: request.agent }));
    if (store === undefined)
        return { allowed: false, reason: 'memory store unavailable' };
    if (optionalLlm(deps.ctx) === undefined)
        return { allowed: false, reason: 'llm service unavailable' };
...
export async function distillTurn(deps, request) {
    const skip = { status: 'skipped', created: 0, merged: 0, rejected: 0, tokensIn: 0, tokensOut: 0, recordIds: [] };
    ...
    const store = deps.registry.open(deps.resolver.resolve({ agent: request.agent }));
    if (store === undefined)
        return { ...skip, reason: 'memory store unavailable' };
    ...
    const auditOnce = (created, timedOut) => {
        if (audited)
            return;
        audited = true;
    ...
    try {
        ...
        try {
            ...
            if (controller.signal.aborted)
                timedOut = true;
            const finishKind = finish !== null && typeof finish === 'object' && 'kind' in finish
                ? String(finish.kind)
                : String(finish);
            if (finishKind !== 'stop') {
                const failure = finish !== null && typeof finish === 'object' && 'failure' in finish
                    ? finish.failure
                    : undefined;
                finishNote =
        ...
        if (created + merged > 0) {
            ...
            log('info', `memory: distilled ${created} new + ${merged} merged record(s) from turn ${request.turn} via ${route.provider}/${route.model}`);
        }
        return {
...
export function buildPrompt(request) {
    ...
    request.signals.slice(0, MAX_SIGNALS_IN_PROMPT).forEach((signal, index) => {
        ...
        if (signal.tool !== undefined)
            parts.push(`tool=${signal.tool}`);
        if (signal.detail !== undefined)
            parts.push(`detail=${redact(signal.detail, 'redacted', 240)}`);
...
export function parseCandidates(text) {
    ...
    for (const item of parsed) {
        ...
        const tags = Array.isArray(record['tags'])
            ? record['tags'].filter((tag) => typeof tag === 'string').slice(0, 6)
            : [];
        out.push({
...
function audit(db, request, resolvedRoute, tokensIn, tokensOut, created, timedOut) {
    try {
        db.prepare('INSERT INTO distill (session_id, turn, model, prompt_hash, tokens_in, tokens_out, created_count, timed_out, at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(request.sessionId, request.turn, `${resolvedRoute}`, hashish(buildPrompt(request)), tokensIn, tokensOut, created, timedOut ? 1 : 0, new Date().toISOString());
    }
    catch (error) {
