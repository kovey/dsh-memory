...
const LAYER_ENUM = ['project', 'global', 'profile', 'episodic'];
/**
...
export function registerTools(ctx, deps) {
    ...
    const register = (definition) => {
        try {
            ...
            log('warn', `memory: registering tool ${definition.name} failed — it is unavailable and the protocol section will not advertise it:`, error);
        }
    };
 ...
 */
function chargeToolOutput(deps, agent, tool, text) {
    ...
    if (charge.allowed)
        return undefined;
    log('info', `memory: ${tool} output refused — session tool budget exhausted (${charge.used}/${budget} tok, this answer would add ~${cost})`);
    return [
...
function targetStores(deps, agent, scope) {
    ...
    if (scope !== 'global' && primary !== undefined)
        stores.push(primary);
...
function recallTool(deps) {
    return defineTool({
        name: 'memory_recall',
        description: 'Fetch the memory pack for a task in one call: project memory plus keyword-matched global lessons, already ranked and trimmed to a token budget. Use it when you want the full recall set explicitly (the same pack is injected automatically at the start of each turn).',
        parameters: {
            task: {
                type: 'string',
                required: true,
        ...
        async execute(args, exec) {
            ...
            if (query.terms.length === 0)
                return 'task text contained no searchable terms';
            // A model-supplied budget must not blow up the calling context.
            const budgetTokens = args.budgetTokens !== undefined
...
function searchTool(deps) {
    return defineTool({
        name: 'memory_search',
        description: 'Search the agent memory store (project memory first, then global). Returns titles, metadata and short excerpts — use memory_get for a full body. Call this before starting non-trivial work and whenever prior experience with the same tool, error or workflow may exist.',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            const filter = {
                ...(args.layer !== undefined ? { layers: [args.layer] } : {}),
            ...
            const lines = [`${merged.length} memory record(s) for "${args.query}":`];
    ...
    return defineTool({
        ...
        parameters: {
            ...
            scope: {
                type: 'string',
                enum: ['auto', 'project', 'global', 'all'],
                description: 'auto = the scope owning this session plus global; project = this session only; global = global memory only; all = the same as auto.',
            },
        },
        output: { schema: TEXT_OUTPUT, render: (_args, value) => [{ type: 'text', text: value }] },
        async execute(args, exec) {
...
function reindexTool(deps) {
    return defineTool({
        name: 'memory_reindex',
        description: 'Rebuild the derived SQLite index from the git-tracked text view (lessons/*.md, metrics.jsonl) for this project or globally. Read-only with respect to memory content; use after editing lesson files by hand or after a git pull.',
        parameters: {
            scope: {
                type: 'string',
        ...
        async execute(args, exec) {
            ...
            if (args.scope === 'global' || args.scope === 'all')
                targets.push(deps.resolver.globalScope());
            const lines = [];
            if (args.embeddings === true) {
                ...
                for (const scope of targets) {
                    ...
                    lines.push(`${scope.kind} (${scope.root}): embedded ${embedded} record(s) — indexed ${stats.indexed}, pending ${stats.pending}${provider.lastError() !== undefined ? ` (last error: ${provider.lastError()})` : ''}`);
                }
                return lines.join('\n') || 'no scope resolved';
            }
            for (const scope of targets) {
                if (args.rebuild === true) {
                    ...
                    lines.push(`${scope.kind} (${scope.root}): rebuilt — imported ${result.imported}, removed ${result.removed}, metrics ${result.metrics}, episodes ${result.episodes}${result.errors.length > 0 ? ` (errors: ${result.errors.join('; ')})` : ''}`);
                    continue;
                }
            ...
            if (lines.length === 0)
                lines.push('no scope resolved');
            return lines.join('\n');
        },
    });
}
function formatHit(index, record, scope, score) {
 ...
 */
export function configTool(deps) {
    return defineTool({
        name: 'memory_config',
        description: 'Adjust memory behaviour for the current session only (not persisted). Currently: autoRecall=false stops automatic recall injection while the memory tools keep working. Call without arguments to see the effective settings.',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            const sessionId = sessionIdOf(agent);
            if (sessionId === undefined)
                return 'no session context — memory_config needs an agent session';
 ...
 * worth keeping here.
 */
export function importTool(deps) {
    return defineTool({
        name: 'memory_import',
        description: 'Import one lesson markdown file (with the memory frontmatter) into this project or the global store. Use when a lesson was written outside the memory root, e.g. copied from another project. Returns the record id and how it was absorbed.',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            if (!fs.existsSync(file) || !fs.statSync(file).isFile())
                return `rejected: ${file} is not a readable file`;
            let text;
            try {
            ...
            const scope = deps.resolver.resolve({ agent, ...(args.layer === 'global' ? { explicit: 'global' } : {}) });
            const store = deps.registry.open(scope);
            if (store === undefined)
                return 'memory store unavailable (SQLite driver missing or memory root unwritable)';
            const id = path.basename(file, '.md');
            if (id.trim() === '')
                return 'rejected: the file name must yield a record id';
