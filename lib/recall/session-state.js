...
export class SessionState {
    ...
    hasInjected(sessionId, recordId) {
        return this.injected.get(sessionId)?.has(recordId) ?? false;
    }
    markInjected(sessionId, recordIds) {
    ...
    markSessionStart(sessionId, at = Date.now()) {
        if (this.startedAt.has(sessionId))
            return;
    ...
    chargeDistill(sessionId, tokens) {
