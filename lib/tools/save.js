...
export function parseTtl(ttl, now = new Date()) {
    if (ttl === undefined || ttl === '' || ttl === 'permanent')
        return undefined;
 ...
 */
export function subagentWriteRefusal(operation) {
    return [
 ...
 */
export function refuseWrite(deps, agent, operation) {
    return deps.resolver.mayWrite(agent) ? undefined : subagentWriteRefusal(operation);
}
export function saveTool(deps) {
    return defineTool({
        name: 'memory_save',
        description: 'Record a durable lesson in agent memory. Use it when a real trap was hit and fixed, when the user explicitly asks you to remember something, or when a workflow proved reliably better. Defaults to project memory; use layer "global" only for cross-project toolchain facts (harness/plugin behaviour, sandbox rules, generic scripts). The write is deduplicated and merged with similar existing lessons.',
        parameters: {
            title: { type: 'string', required: true, description: 'Short lesson title (becomes the record id).' },
            body: {
                type: 'string',
                required: true,
                description: 'Trigger situation + the correct action (include the exact command, flag or path). Vague advice is rejected.',
            },
            layer: {
                type: 'string',
                enum: ['project', 'global', 'profile'],
                description: 'Where to store it. Default: project. Global is only for cross-project toolchain facts; profile (L5) is for a standing preference the user stated — it is also written to profile/preferences.md and stays resident in the prompt.',
            },
        ...
        async execute(args, exec) {
            ...
            if (title.length < 4)
                return 'rejected: title is too short to be a useful lesson id';
            ...
            const scope = deps.resolver.resolve({ agent, ...(explicitGlobal ? { explicit: 'global' } : {}) });
            ...
            if (args.layer === 'profile') {
                ...
                if (file === undefined)
                    return 'rejected: could not write the profile layer for this scope';
            }
            const evidenceKind = (args.evidence ?? 'self-report');
            const evidence = [
                {
                    kind: evidenceKind,
            ...
            const draft = {
                ...
                ...(parseTtl(args.ttl) !== undefined ? { expiresAt: parseTtl(args.ttl) } : {}),
                evidence,
                origin: 'user',
