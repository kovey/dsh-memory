...
export interface DecayPlan {
    ...
    archive: { record: MemoryRecord; reason: string }[]
...
export function planDecay(db: DatabaseSync, now = new Date()): DecayPlan {
    ...
    const archive: { record: MemoryRecord; reason: string }[] = []
    for (const record of records) {
        ...
        if (record.confidence > STALE_PENDING_CONFIDENCE) continue
        if (record.timesRecalled > 0) continue
...
export function applyDecay(
    ...
    transact(db, () => {
        ...
        for (const item of plan.archive) archiveStatement.run(now.toISOString(), item.record.id)
        ...
        if (plan.factor < 1) {
            ...
            for (const record of survivors) {
                ...
                if (next === record.confidence) continue
                decayStatement.run(next, record.id)
                outcome.decayed += 1
            }
        }
        if (plan.factor < 1 || plan.elapsedDays === 0) setMeta(db, LAST_DECAY_KEY, now.toISOString())
    })
    return outcome
}

/** Timestamp of the last consolidation run, if any. */
export function lastConsolidateAt(db: DatabaseSync): Date | undefined {
...
export function markConsolidated(db: DatabaseSync, now = new Date()): void {
...
export function consolidationDue(
    db: DatabaseSync,
    options: { everyDays: number; everyNTasks: number; now?: Date },
    ...
    if (days >= options.everyDays) return { due: true, reason: `${Math.floor(days)}d since last run` }
    ...
    if (tasks >= options.everyNTasks) return { due: true, reason: `${tasks} tasks since last run` }
    return { due: false, reason: 'not due' }
}

export function scopeLabelOf(scope: MemoryScope): string {
