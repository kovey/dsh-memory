...
export function protocolText(config, capabilities = { save: false }) {
    const lines = [
        '记忆系统（dsh-memory）已启用：',
...
export function registerProtocolSection(ctx, deps) {
    if (!deps.config.prompt.protocol.enabled)
        return [];
    try {
 ...
 */
export function registerAgentIndexSection(agent, deps) {
    if (!deps.config.prompt.indexSummary.enabled)
        return false;
    const ctx = agent.ctx;
    if (ctx === undefined)
        return false;
...
export function indexSummaryText(deps, agent) {
    try {
        ...
        if (scope.kind === 'project') {
            ...
            if (global !== undefined) {
                ...
                if (globalCounts.total > 0)
                    lines.push(`全局记忆：${globalCounts.total} 条跨项目工具链教训（命中关键词时自动召回）。`);
            }
        }
...
function clampTokens(text, budgetTokens) {
    if (budgetTokens <= 0)
        return '';
