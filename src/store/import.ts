...
import type { EvidenceCount, ParsedLesson } from './frontmatter.js'
...
import type { Evidence, Layer, MemoryRecord, MemoryScope, RecordStatus } from './types.js'
...
export function importLessons(db: DatabaseSync, scope: MemoryScope, options: ImportOptions = {}): ImportResult {
    const result: ImportResult = { root: scope.root, scanned: 0, imported: 0, merged: 0, skipped: 0, errors: [] }
        ...
        for (const name of entries) {
            ...
            try {
                ...
                if (existing !== undefined && existing.id !== id) {
                    if (mergeIntoExisting(db, existing, record)) result.merged += 1
...
function storedIds(db: DatabaseSync): { exact: Set<string>; canonical: Map<string, string> } {
 ...
 * stays.
 */
function preferCanonical(index: Map<string, string>, id: string): void {
    const key = normalizeDashes(id)
    const kept = index.get(key)
    if (kept === undefined || (kept !== key && id === key)) index.set(key, id)
}
 ...
 */
function mergeIntoExisting(db: DatabaseSync, existing: MemoryRecord, candidate: MemoryRecord): boolean {
...
function countByKey(evidence: readonly Evidence[]): Map<string, number> {
    const counts = new Map<string, number>()
    for (const item of evidence) counts.set(evidenceKey(item), (counts.get(evidenceKey(item)) ?? 0) + 1)
    return counts
}
 ...
 */
function fillEvidenceGaps(existing: readonly Evidence[], counts: readonly EvidenceCount[], at: string): Evidence[] {
    const have = new Map<string, number>()
    for (const item of existing) have.set(item.kind, (have.get(item.kind) ?? 0) + 1)
    const rows: Evidence[] = []
    for (const { kind, count } of counts) {
        ...
        for (let i = 0; i < missing && rows.length < MAX_EVIDENCE_TOTAL; i += 1) {
            rows.push({ kind: kind as Evidence['kind'], at })
        }
    }
...
function evidenceFromSummary(summary: string | undefined, at: string): Evidence[] {
